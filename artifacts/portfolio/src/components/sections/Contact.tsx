...
export function Contact() {
  ...
  const email      = profile?.email       || "ashis@example.com";
  const githubUrl  = profile?.github_url  || "https://github.com/ashis";
  const linkedinUrl= profile?.linkedin_url || "https://linkedin.com/in/ashis";
  const githubHandle  = githubUrl.replace(/^https?:\/\/(www\.)?/, "");
  const linkedinHandle= linkedinUrl.replace(/^https?:\/\/(www\.)?/, "");
    ...
    <section id="contact" className="py-24">
      <div className="mb-12 fade-up">
        ...
        <p className="mt-4 max-w-2xl text-lg text-secondary-foreground">
          If you want to review my work, source code, or connect professionally, use the links below.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3 mb-8">
        <div className="rounded-3xl border border-border bg-card p-8 shadow-md fade-up">
          ...
          <p className="mt-4 text-xl font-semibold break-all">{email}</p>
          <button
            onClick={copyEmail}
        ...
        <div className="rounded-3xl border border-border bg-card p-8 shadow-md fade-up" style={{ transitionDelay: "80ms" }}>
          ...
          <p className="mt-4 text-xl font-semibold truncate">{githubHandle}</p>
          <a
            href={githubUrl}
            target="_blank"
            rel="noreferrer"
        ...
        <div className="rounded-3xl border border-border bg-card p-8 shadow-md fade-up" style={{ transitionDelay: "160ms" }}>
          ...
          <p className="mt-4 text-xl font-semibold truncate">{linkedinHandle}</p>
          <a
            href={linkedinUrl}
            target="_blank"
            rel="noreferrer"
      ...
      <div className="rounded-3xl border border-border bg-card p-8 shadow-md fade-up" style={{ transitionDelay: "240ms" }}>
        ...
        <p className="text-sm text-muted-foreground mb-8">
          Prefer to write directly? Fill out the form and it will open in your email client.
        </p>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            <button type="submit" className="min-h-11 rounded-xl bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground">
              Send Message
            </button>
          </div>
        </form>
